                ...
                <label>
                    ...
                    <span class='CFBetter_setting_menu_label_text'>使用ChatGPT翻译(API)
                        <div class="help_tip">
                            ...
                            <p>Codeforces Better!默认使用 gpt-3.5-turbo 模型进行翻译，脚本的所有请求均在本地完成</p>
...
async function translate_api2d(raw) {
    ...
    if (is_oldLatex) {
        postData = JSON.stringify({
            model: api2d_model,
            messages: [{ role: 'user', content: '请帮我将下面的文本翻译为中文，这是一道编程竞赛题描述的一部分，注意术语的翻译，注意保持其中的【】、HTML标签本身以及其中的内容不翻译不变动，你只需要回复翻译后的内容即可，不要回复任何其他内容：\n\n' + raw }],
            temperature: 0.7
        });
    } else {
        postData = JSON.stringify({
